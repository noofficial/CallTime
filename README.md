...
- **Manager access:** start the Node API with either `MANAGER_PASSWORD` or
  `MANAGER_PASSWORD_HASH` set in the environment. For example:

  ```bash
  MANAGER_PASSWORD="super-secure-passphrase" npm start
  ```

  Use `MANAGER_PASSWORD_HASH` if you prefer to store a precomputed PBKDF2 hash
  generated by the app (format `salt:hash`).

- **Client access:** managers set or update each campaign’s portal password in
  the client configuration form. Campaign staff sign in from the client portal
  using their numeric campaign ID and the password you provide.
